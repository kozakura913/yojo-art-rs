...
impl APDeliverService{
	pub async fn new(sign: APSignatureService,db:DataBase,redis_for_job_queue: ConnectionManager)->Self{
	...
	pub async fn post(&self,target:impl Iterator<Item=DeliverTarget>,me_id:String,content:serde_json::Value)->Result<(),BoxError>{
		...
		self.post_to_inbox(inbox_urls.into_iter(),me_id,content).await;
		Ok(())
	}
	async fn post_to_inbox<T>(&self,inbox:impl Iterator<Item=T>,me_id:String,content:serde_json::Value) where T:Into<String>{
