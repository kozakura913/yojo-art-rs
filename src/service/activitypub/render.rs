...
use crate::{models::{emoji::MiEmoji, note::MiNote, note_reaction::MiNoteReaction}, service::activitypub::types::{ApEmoji, ApEmojiAuthor, ApImage, ApLike, ApMisskeyLicense, ApSearchableBy, FreeText}, MisskeyConfig};
...
impl ApRenderService{
	...
	pub fn render_like(&self,note:&MiNote,reaction:MiNoteReaction,emoji:Option<MiEmoji>)->ApLike{
		...
		ApLike{
			...
			object: note.uri.clone().unwrap_or_else(||format!("{}notes/{}",self.misskey_config.url,reaction.note_id)),
			content: reaction.reaction.to_owned(),
			_misskey_reaction:reaction.reaction.to_owned(),
			tag,
		}
	}
	pub fn add_context(&self,json:impl Serialize)->Result<serde_json::Value,crate::ServerError>{
