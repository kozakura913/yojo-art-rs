...
use crate::{models::user_keypair::MiUserKeypair, DataBase, ParsedMisskeyConfig};
...
impl APSignatureService {
	...
	pub async fn post(&self,inbox:impl AsRef<str>,me_id:impl AsRef<str>,sha256_digest:&String,body:impl Into<tokio_util::bytes::Bytes>)->Result<reqwest::Response,BoxError>{
		let date=chrono::Utc::now().format("%a, %d %b %Y %H:%M:%S %Z").to_string();
		let body:tokio_util::bytes::Bytes=body.into();
		self.signature(inbox,&date,me_id.as_ref(),Some(&sha256_digest),Some(body.clone())).await
	}
	/*
	...
	*/
	async fn signature(&self,url:impl AsRef<str>,date:&str,user_id:&str,digest:Option<&String>,body:Option<tokio_util::bytes::Bytes>)->Result<reqwest::Response,BoxError>{
		...
		let mut sign_target=format!("(request-target): {request_target}\nhost: {host}\ndate: {date}\n");
		...
		let signature=format!("keyId=\"{key_id}\",algorithm=\"rsa-sha256\",headers=\"{sign_headers}\",signature=\"{signature}\"");
