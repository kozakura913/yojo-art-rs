		...
		if let Some(me_id) = me_id {
			...
			let f_following = async {
				if followings_user.is_none() {
					followings_user.replace(followings(&mut (&self.db).into(), me_id).await?);
				}
			...
			let f_renote_muting = async {
				if renote_muting_user.is_none() {
					renote_muting_user.replace(renote_muting(&mut (&self.db).into(), me_id).await?);
				}
				Ok::<&HashSet<std::string::String>, diesel::result::Error>(
			...
			let f_muted_instances = async {
				if hint_muted_instances.is_none() {
					hint_muted_instances
						.replace(muted_instances(&mut (&self.db).into(), me_id).await?);
				}
				Ok::<&HashSet<std::string::String>, diesel::result::Error>(
...
async fn raw_htl(
	...
	let f_following = async {
		if followings_user.is_none() {
			followings_user.replace(followings(&mut db.into(), me_id).await?);
		}
		Ok::<&HashSet<std::string::String>, diesel::result::Error>(
			followings_user.as_ref().unwrap(),
		)
	...
	let f_muted_instances = async {
		if hint_muted_instances.is_none() {
			hint_muted_instances.replace(muted_instances(&mut db.into(), me_id).await?);
		}
		Ok::<&HashSet<std::string::String>, diesel::result::Error>(
			hint_muted_instances.as_ref().unwrap(),
		)
...
async fn renote_muting(
	...
	let query = renote_muting
		.filter(muterId.eq(me_id))
		.select(MiRenoteMuting::as_select());

	let mi_renote_muting: Vec<MiRenoteMuting> = match con {
		//TODO 重複コードなんとかしたい
	...
	Ok(renote_muting_set)
}
async fn muted_instances(
	con: &mut DBConnectionRef<'_, '_>,
	me_id: &str,
) -> Result<HashSet<String>, diesel::result::Error> {
	...
	for host in res.muted_instances.into_inner() {
...
async fn followings(
	...
	let mi_followings: Vec<String> = {
		...
		use diesel::QueryDsl;
		use diesel_async::RunQueryDsl;
		let query = following.filter(followerId.eq(me_id)).select(followeeId);
		match con {
			//TODO 重複コードなんとかしたい
			DBConnectionRef::Borrowed(con) => query.load(*con).await,
			DBConnectionRef::Mutex(m) => {
